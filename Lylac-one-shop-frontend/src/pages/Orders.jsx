...
// Returns the request configuration used to fetch orders for the logged-in user or email lookup.
const getOrderRequestConfig = ({ backendUrl, token, email, page, itemsPerPage }) => {
  if (token) {
    return {
  ...
  if (email) {
    return {
      url: `${backendUrl}/api/order/userorders`,
      data: { email, page, limit: itemsPerPage }
    };
  }
...
// Renders a single order card with its item summary and payment/shipping details.
const OrderCard = ({ order, currency, formatDate, onTrackOrder, getStatusColor }) => {
  return (
...
// Renders pagination controls for the orders list.
...
const Orders = () => {
  ...
  const loadOrderData = async (page = 1) => {
    try {
      ...
      const requestConfig = getOrderRequestConfig({
        backendUrl,
        token,
        email,
        page,
      ...
      const response = await axios.post(
        requestConfig.url,
        requestConfig.data,
        requestConfig.headers ? { headers: requestConfig.headers } : undefined
      );
  ...
  const getStatusColor = (status = '') => {
    return ORDER_STATUS_STYLES[status.toLowerCase()] || 'bg-gray-500';
  }
            ...
            <div className="space-y-8">
              {orders.map((order, index) => (
                <OrderCard
                  ...
                  onTrackOrder={() => loadOrderData(currentPage)}
                />
              ))}
            </div>
          )}
