...
export async function getProduct(friendlyUrl: string) {
...
// POST /api/v1/search
export async function searchProducts({ query, count = 20 }: { query?: string; count?: number } = {}) {
  try {
...
export async function listCategories() {
  ...
  const arr: any[] = data?.categories || data?.list || [];
...
export async function listManufacturers() {
  try {
    ...
    return (data?.manufacturers || []).map((m: any) => ({ id: m.id, code: m.code, name: m?.description?.name || m.code }));
  } catch { return []; }
}

// GET /api/v1/product/{id}/reviews
export async function listReviews(id: string) {
  try {
