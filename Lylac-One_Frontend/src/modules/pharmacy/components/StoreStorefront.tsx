...
export function StoreStorefront({ store, isLive: _isLive }: { store: any; isLive?: boolean }) {
        ...
        <div className="container flex items-center gap-3 h-16">
          ...
          <div className="flex-1 hidden md:block max-w-lg mx-4">
            <div className="relative">
              ...
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder={`Search medicines at ${store.name.split(' ')[0]}…`} className="pl-10 h-10 rounded-full bg-muted/60 border-none focus-visible:ring-2" style={{ '--tw-ring-color': theme.primary } as React.CSSProperties} />
            </div>
          </div>
...
function SectionTitle({ title, subtitle, theme, small = false, centered = false }) {
  return (
...
function ContactRow({ icon: Icon, label, value, color, href = undefined }) {
...
function NotFoundBlock({ store, query = '', theme, generic = false, onSearchAll, onNotify }) {
  return (
