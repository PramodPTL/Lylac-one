...
export function MedicineDetailView({
  medicineId,
  onBack,
  cart,
  onAddToCart,
  ...
  // Keep product identity aligned with the server cart.
  ...
  const qty = medicine ? cart.find((c) => c.id === medicine.id)?.qty || 0 : 0;
  const alternatives = liveList
    .filter((m) => m.id !== medicine?.id && m.category === medicine?.category)
    .slice(0, 4);
  const [selectedStore, setSelectedStore] = useState(PHARMACIES[0].id);
  const isWishlisted = medicine && wishlist?.includes(medicine.id);
  const inCompare = medicine && compare?.includes(medicine.id);

  if (liveProductsQ.isLoading)
    return (
      <div className="container py-16 text-center">Loading medicine...</div>
    );
  if (!medicine)
    return (
      <div className="container py-16 text-center">
        <h1 className="text-xl font-bold">Medicine not found</h1>
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-border">
        <div className="container flex items-center justify-between py-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={() => addToCompare?.(medicine.id)}
              className={`p-2 rounded-full hover:bg-muted ${inCompare ? "text-primary" : ""}`}
              aria-label="Compare"
            >
              <svg
                width="20"
        ...
        <div>
          ...
          <div className="mt-3 flex gap-2">
            {[medicine.image, medicine.image, medicine.image].map((img, i) => (
              <div
                key={i}
                className={`w-16 h-16 rounded-lg overflow-hidden border-2 ${i === 0 ? "border-primary" : "border-border"}`}
              >
        ...
        <div>
          {medicine.rx && (
            <Badge variant="secondary" className="mb-2 gap-1">
              <Pill className="w-3 h-3" /> Prescription Required
            </Badge>
          )}
          ...
          <div className="mt-4 p-4 rounded-2xl border border-border bg-accent/30">
            ...
            <div className="text-sm text-muted-foreground mt-1">
              Inclusive of all taxes • {medicine.pack}
            </div>
          </div>
          ...
          <div className="mt-4">
            <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
              <StoreIcon className="w-4 h-4" /> Available at nearby stores
            </h4>
            <div className="space-y-2">
              {PHARMACIES.slice(0, 3).map((p) => (
                <button
                  key={p.id}
                  onClick={() => setSelectedStore(p.id)}
                  className={`w-full flex items-center gap-3 p-3 rounded-xl border-2 transition text-left ${selectedStore === p.id ? "border-primary bg-accent" : "border-border hover:bg-muted/50"}`}
                >
                  <div
                    className={`w-2 h-2 rounded-full ${selectedStore === p.id ? "bg-primary" : "bg-border"}`}
                  />
                  <div className="flex-1">
                    <div className="font-semibold text-sm">{p.name}</div>
                    <div className="text-xs text-muted-foreground flex items-center gap-2">
                      <MapPin className="w-3 h-3" />
                      {p.distance} • <Clock className="w-3 h-3" />
                      {p.deliveryTime}
                    </div>
                  </div>
      ...
      <div className="container mt-8">
        <Tabs defaultValue="uses">
          ...
          <TabsContent value="uses" className="pt-4">
            <InfoBlock icon={Info} title="Medical Uses" text={medicine.uses} />
          </TabsContent>
              ...
              {[
                {
                  n: "Priya S.",
                  r: 5,
                  t: "Works great, fast delivery from nearby store!",
                },
                {
                  n: "Rahul M.",
                  r: 4,
                  t: "Genuine product. Packaging was excellent.",
                },
              ].map((rv, i) => (
          ...
          {qty > 0 ? (
            <>
              ...
              <Button
                ...
                className="rounded-full flex-1 bg-secondary hover:bg-secondary/90"
              >
                Go to Cart
              </Button>
            </>
            ...
            <>
              <Button
                ...
                variant="outline"
                onClick={() => onAddToCart(medicine, 1)}
                className="rounded-full border-primary text-primary flex-1"
              >
                Add to Cart
              </Button>
              <Button
                ...
                className="rounded-full bg-secondary hover:bg-secondary/90 flex-1"
              >
                Buy Now
              </Button>
            </>
...
function InfoBlock({ icon: Icon, title, text, tone = "default" }) {
  return (
    <div
      className={`p-4 rounded-2xl border ${tone === "warning" ? "border-amber-200 bg-amber-50" : "border-border bg-muted/30"}`}
    >
      <div className="flex items-center gap-2 font-semibold text-sm mb-2">
        <Icon
          className={`w-4 h-4 ${tone === "warning" ? "text-amber-600" : "text-primary"}`}
        />
        {title}
      </div>
