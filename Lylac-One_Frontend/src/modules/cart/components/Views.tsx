...
import { MedicineCard, CategoryCard } from "../../store/components/Cards";
...
export function CartView({ cart, onAddToCart, onRemove, onNavigate, onBack }) {
  ...
  const applyCoupon = () => {
    if (
      ["HEALTH25", "FIRST100", "FREEDEL", "B2G1"].includes(coupon.toUpperCase())
    ) {
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-muted-foreground mt-2 max-w-sm">
          Explore nearby pharmacies and add medicines to your cart.
        </p>
        <Button
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-amber-800 dark:text-amber-300 mt-0.5">
                    Some items need a valid prescription. Upload during
                    checkout.
                  </div>
                </div>
                <Button
            ...
            <div className="flex items-center gap-2 mb-3 pb-3 border-b border-border">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-muted-foreground flex items-center gap-1">
                  <Truck className="w-3 h-3" /> Delivery in{" "}
                  {PHARMACIES[0].deliveryTime}
                </div>
              </div>
          ...
          <Card className="p-4">
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder="Enter code (try HEALTH25)"
                className="h-10"
              />
              <Button
                onClick={applyCoupon}
            ...
            {applied && (
              <div className="mt-2 text-xs text-success font-semibold flex items-center gap-1">
                <Check className="w-3 h-3" /> {applied} applied — Extra 10% off
              </div>
            )}
          </Card>
        </div>
          ...
          <Card className="p-5">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="text-xs text-success font-medium flex items-center gap-1 mt-1">
                <Percent className="w-3 h-3" /> You saved ₹
            ...
            <Button
              ...
              size="lg"
              className="w-full mt-4 rounded-full bg-secondary hover:bg-secondary/90 font-semibold h-12"
            >
      ...
      <div className="container py-4 grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <Card className="p-5">
            ...
            >
              {ADDRESSES.map((a) => (
                <label
                  key={a.id}
                  className={`flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer ${address === a.id ? "border-primary bg-accent" : "border-border"}`}
                >
            ...
            <Button
              variant="outline"
              className="mt-3 rounded-full w-full sm:w-auto"
            >
              + Add New Address
            </Button>
          </Card>
          <Card className="p-5">
            ...
            >
              {[
                ...
                <label
                  key={s.v}
                  className={`p-3 rounded-xl border-2 cursor-pointer ${slot === s.v ? "border-primary bg-accent" : "border-border"}`}
                >
          ...
          <Card className="p-5">
            <SectionTitle
              icon={Upload}
              title="3. Upload Prescription (optional)"
            />
              ...
              {[
                {
                  v: "upi",
                  l: "UPI",
                  s: "Pay via GPay, PhonePe, Paytm",
                  icon: Wallet,
                },
                {
                  v: "card",
                  l: "Credit / Debit Card",
                  s: "Visa, Mastercard, RuPay",
                  icon: CreditCard,
                },
                {
                  v: "cod",
                  l: "Cash on Delivery",
                  s: "Pay when delivered",
                  icon: Package,
                },
              ].map((p) => {
                const I = p.icon;
                return (
                  <label
                    key={p.v}
                    className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer ${payment === p.v ? "border-primary bg-accent" : "border-border"}`}
                  >
        ...
        <div className="lg:sticky lg:top-20 h-fit">
          <Card className="p-5">
            ...
            <Button
              ...
              disabled={busy}
              size="lg"
              className="w-full mt-4 rounded-full bg-secondary hover:bg-secondary/90 h-12 font-semibold"
            >
              {busy ? "Processing..." : `Place Order • ₹${total}`}
            </Button>
            <div className="mt-3 flex items-center gap-1.5 text-[11px] text-muted-foreground justify-center">
              <ShieldCheck className="w-3 h-3" /> 100% Safe & Secure Payments
            </div>
          </Card>
        </div>
      </div>
...
export function OrdersView({ onBack, onOpenOrder }) {
  const ordersQuery = useQuery({
    queryKey: ["orders"],
    queryFn: () => orderApi.listOrders({ count: 25 }),
    retry: false,
  });
    ...
    <div className="pb-24 md:pb-8">
      <div className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b border-border">
        <div className="container flex items-center gap-2 py-3">
          <button
            onClick={onBack}
            className="p-2 -ml-2 rounded-full hover:bg-muted md:hidden"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
      ...
      <div className="container py-4 space-y-3 max-w-3xl">
        ...
        {orders.map((o) => (
          <Card
            key={o.id}
            className="p-4 hover:shadow-card transition cursor-pointer"
          ...
          <Card className="overflow-hidden">
            <div className="h-56 bg-gradient-to-br from-teal-100 via-emerald-50 to-cyan-100 dark:from-teal-950 dark:via-emerald-950 dark:to-cyan-950 relative">
              ...
              <div className="absolute bottom-6 right-8 flex flex-col items-center">
                <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center pulse-ring">
            ...
            <div className="relative pl-6">
              ...
              {order.timeline.map((t, i) => (
                <div key={i} className="relative pb-4 last:pb-0">
                  <div
                    className={`absolute -left-6 top-0.5 w-4 h-4 rounded-full border-2 ${t.done ? "bg-primary border-primary" : "bg-background border-border"}`}
                  >
                    {t.done && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground flex items-center gap-1">
                    ...
                    {order.partner.rating} • {order.partner.vehicle}
                  </div>
                </div>
              </div>
...
export function ProfileView({ onBack, onNavigate }) {
  const { profile, isAuthed, logout } = useAuth();
  const profileName =
    [profile?.firstName, profile?.lastName].filter(Boolean).join(" ") ||
    profile?.name ||
    "Your profile";
  const profileEmail =
    profile?.emailAddress || profile?.email || "Sign in to manage your account";
  const groups = [
    {
      t: "Orders & Prescriptions",
      items: [
        {
          icon: Package,
          label: "My Orders",
        ...
        },
        {
          ...
          sub: "4 uploads",
          route: "prescription-history",
        },
        {
          icon: Upload,
          label: "Uploaded Prescriptions",
          sub: "Manage files",
          route: "uploaded-prescriptions",
        },
        {
          icon: MapPinned,
          label: "Saved Pharmacies",
          sub: "3 stores",
          route: "saved-pharmacies",
        },
      ],
    },
    {
    ...
    {
      t: "",
      items: [{ icon: LogOut, label: "Logout", danger: true, route: "logout" }],
    },
  ];
      ...
      <div className="container py-4 max-w-3xl">
        <Card className="p-5 gradient-hero text-white">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="text-xl font-bold">{profileName}</div>
              <div className="text-sm opacity-90">{profileEmail}</div>
              <button
                onClick={() => onNavigate("profile-sub", "edit-profile")}
          ...
          <div className="grid grid-cols-3 gap-3 mt-5 pt-4 border-t border-white/20">
            ...
            >
              ...
              <div className="text-[11px] opacity-90">Orders</div>
            </button>
            <button className="text-center">
              <div className="text-lg font-bold">₹2,450</div>
              <div className="text-[11px] opacity-90">Saved</div>
            </button>
            <button
              onClick={() => onNavigate("profile-sub", "loyalty")}
              className="text-center"
            >
              <div className="text-lg font-bold">1,240</div>
              <div className="text-[11px] opacity-90">Lylac Points</div>
            </button>
          </div>
        </Card>
          ...
          {[
            {
              ...
              i: Heart,
              r: "wishlist",
              c: "bg-rose-100 dark:bg-rose-950 text-rose-600",
            },
            {
              l: "Recent",
              i: Clock,
              r: "recently-viewed",
              c: "bg-blue-100 dark:bg-blue-950 text-blue-600",
            },
            {
              l: "Compare",
              i: SlidersHorizontal,
              r: "compare",
              c: "bg-purple-100 dark:bg-purple-950 text-purple-600",
            },
            {
              l: "Reminders",
              i: Pill,
              r: "reminders",
              c: "bg-amber-100 dark:bg-amber-950 text-amber-600",
            },
          ].map((q) => {
            const I = q.i;
            return (
              <button
                ...
                onClick={() =>
                  ...
                  q.r === "wishlist"
                    ? onNavigate(q.r)
                    : onNavigate("profile-sub", q.r)
                }
                className="p-3 rounded-xl border border-border hover:shadow-card transition flex flex-col items-center gap-1"
              >
                <div
                  className={`w-10 h-10 rounded-lg ${q.c} flex items-center justify-center`}
                >
                  <I className="w-4 h-4" />
                </div>
        ...
        {groups.map((g, gi) => (
          <div key={gi} className="mt-6">
            {g.t && (
              <h3 className="text-xs font-bold text-muted-foreground uppercase mb-2 px-1">
                {g.t}
              </h3>
            )}
            <Card className="divide-y divide-border overflow-hidden">
              {g.items.map((it, i) => {
                ...
                const handle = () => {
                  ...
                  if (["orders", "wishlist", "settings"].includes(it.route))
                    return onNavigate(it.route);
                ...
                return (
                  <button
                    key={i}
                    onClick={handle}
                    className={`w-full flex items-center gap-3 p-4 hover:bg-muted/50 transition text-left ${it.danger ? "text-destructive" : ""}`}
                  >
                    <div
                      className={`w-9 h-9 rounded-lg ${it.danger ? "bg-destructive/10 text-destructive" : "bg-accent text-primary"} flex items-center justify-center`}
                    >
                      <I className="w-4 h-4" />
                    </div>
                    <div className="flex-1">
          ...
          {CATEGORIES.map((c) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-center gap-2 p-3 rounded-2xl border border-border hover:shadow-card transition bg-card"
              >
                <div
                  className={`w-14 h-14 rounded-xl ${c.color} flex items-center justify-center`}
                >
    ...
    <div className="pb-24 md:pb-8">
      ...
      <div className="container py-4 max-w-2xl">
        <Card className="p-4 border-primary/30 bg-accent/30">
          <button
            onClick={() => {
              onSelect("Koramangala, Bengaluru");
...
export function StoreView({
  ...
  const meds = MEDICINES.filter((m) => {
    if (q && !`${m.name} ${m.brand}`.toLowerCase().includes(q.toLowerCase()))
      return false;
  ...
  return (
    <div className="pb-24 md:pb-8">
      ...
      <div className="relative h-44 md:h-52">
        ...
        <button
          onClick={onBack}
          className="absolute top-4 left-4 p-2 rounded-full bg-white/95 shadow-lg"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <button
          onClick={() => {
            setSaved(!saved);
            toast.success(saved ? "Removed from saved" : "Pharmacy saved!");
          }}
          className="absolute top-4 right-4 p-2 rounded-full bg-white/95 shadow-lg"
        >
          <Heart
            className={`w-5 h-5 ${saved ? "fill-rose-500 text-rose-500" : ""}`}
          />
        </button>
        <div className="absolute bottom-3 inset-x-0 container text-white">
          <div className="flex items-end gap-3">
            <div className="w-14 h-14 rounded-2xl bg-white text-primary text-2xl font-bold flex items-center justify-center shadow-xl">
              {store.logo}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center flex-wrap gap-2 mt-1 text-[11px]">
                <span className="flex items-center gap-1 bg-success px-2 py-0.5 rounded-full font-bold">
                ...
                <span
                  className={`px-2 py-0.5 rounded-full font-semibold ${store.isOpen ? "bg-success/90" : "bg-destructive/90"}`}
                >
      ...
      <div className="container mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
        {[
          ...
          return (
            <div
              key={s.l}
              className="p-3 rounded-xl bg-muted/50 flex items-center gap-2 text-xs"
            >
      ...
      <div className="sticky top-16 md:top-16 z-20 bg-background/95 backdrop-blur border-b border-border mt-4">
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full h-11 pl-10 rounded-full bg-muted/70 border-none outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <label className="flex items-center gap-2 text-sm">
                      <Checkbox
                        ...
                        onCheckedChange={(c) =>
                          setFilters({ ...filters, inStock: c === true })
                        }
                      />{" "}
                      In Stock only
                    </label>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {[0, 10, 20, 30].map((d) => (
                        <button
                          ...
                          className={`px-3 py-2 rounded-lg text-xs font-medium border ${filters.discount === d ? "bg-primary text-primary-foreground border-primary" : "border-border hover:bg-muted"}`}
                        >
                          {d === 0 ? "Any" : `${d}%+`}
                        </button>
                      ))}
        ...
        <div className="container pb-3">
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            {storeCats.map((c) => {
              const iconName = "icon" in c ? c.icon : undefined;
              const Icon = iconName ? Icons[iconName] || null : null;
              const active = activeCat === c.id;
              return (
                <button
                  key={c.id}
                  onClick={() => setActiveCat(c.id)}
                  className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition ${active ? "bg-primary text-primary-foreground" : "bg-muted text-foreground hover:bg-accent"}`}
                >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {meds.map((m) => (
              <MedicineCard
                ...
                onAdd={() => onAddToCart(m, 1)}
                onInc={() => onAddToCart(m, 1)}
                onDec={() => onAddToCart(m, -1)}
    ...
    <div className="pb-24 md:pb-8">
      <div className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b border-border">
        ...
        <div className="container pb-3 flex gap-2">
          ...
          <select
            ...
            className="h-10 rounded-full px-3 border border-border bg-background text-sm"
          >
      ...
      <div className="container py-4">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {pharmacies.map((p) => (
            <StoreCard key={p.id} store={p} onClick={() => onOpenStore(p.id)} />
          ))}
        </div>
      </div>
    </div>
