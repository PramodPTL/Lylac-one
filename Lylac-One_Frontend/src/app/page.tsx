...
import { Navbar, BottomNav, Footer } from "@/modules/store/components/AppShell";
import { HomeView } from "@/modules/marketplace/components/HomeView";
import { SearchView } from "@/modules/search/components/SearchView";
import { MedicineDetailView } from "@/modules/medicine/components/MedicineDetailView";
import {
  CartView,
  CheckoutView,
  OrdersView,
  ...
  LocationView,
  AllPharmaciesView,
} from "@/modules/cart/components/Views";
import { AuthView } from "@/modules/auth/components/AuthView";
import { SettingsView } from "@/modules/customer/components/SettingsView";
import {
  ProfileSubView,
  CompareView,
  RecentlyViewedView,
} from "@/modules/customer/components/ProfileSubViews";
...
import { cartCodeStore } from "@/modules/cart/services/cart.service";
...
function InnerApp() {
  ...
  const [currentStoreId, setCurrentStoreId] = usePersistedState(
    "lylac-current-store",
    null,
  );
  ...
  const addToCart = async (medicine, delta) => {
    let nextQty = 0;
    setCart((c) => {
      const idx = c.findIndex((i) => i.id === medicine.id);
      if (idx === -1 && delta > 0) {
        toast.success(`${medicine.name} added to cart`);
        return [...c, { ...medicine, qty: delta }];
      }
      if (idx === -1) return c;
      const newQty = c[idx].qty + delta;
      nextQty = Math.max(0, newQty);
      if (newQty <= 0) {
    ...
    if (!medicine?.isLive) return;
    try {
      const code = cartCodeStore.get();
      if (nextQty <= 0 && code)
        ...
        await cartApi.updateCartItem({
          ...
          quantity: nextQty || delta,
        });
      else if (delta > 0)
        await cartApi.createCart({ productId: medicine.id, quantity: delta });
    } catch (error) {
      toast.error(error?.message || "Could not update the pharmacy cart");
    }
  };
  const removeFromCart = async (id) => {
    ...
    if (item?.isLive && cartCodeStore.get()) {
      try {
        await cartApi.removeCartItem({
          code: cartCodeStore.get(),
          sku: item.sku || item.id,
        });
      } catch (error) {
        toast.error(
          error?.message || "Could not remove item from pharmacy cart",
        );
      }
    }
  ...
  const placeOrder = async (payload = {}) => {
    const code = cartCodeStore.get();
    if (!code)
      throw { message: "Your cart is not connected to the pharmacy API." };
    const confirmation = await orderApi.checkoutCart({
      code,
      authed: !!tokenStore.get(),
      payload,
    });
    const order = confirmation?.order || confirmation;
    setCart([]);
    setCurrentStoreId(null);
    cartCodeStore.clear();
    setView("tracking");
    setSelectedOrder(order?.id || order?.orderId);
    return order;
  };
  ...
  const renderView = () => {
    switch (view) {
      case "home":
        return <HomeView {...commonProps} location={location} />;
      case "search":
      ...
      case "all-pharmacies":
        return <AllPharmaciesView onBack={back} onOpenStore={openStore} />;
      case "cart":
        return (
          <CartView
            cart={cart}
      ...
      case "orders":
        return <OrdersView onBack={back} onOpenOrder={openOrder} />;
      case "tracking":
        return <OrderTrackingView orderId={selectedOrder} onBack={back} />;
      case "profile":
        return <ProfileView onBack={back} onNavigate={navigate} />;
      case "categories":
        return <CategoriesView onBack={back} onNavigate={navigate} />;
      case "location":
        return (
          <LocationView
            onBack={back}
      ...
      case "upload":
        return (
          <div className="container py-12 text-center max-w-lg mx-auto">
            <div className="w-20 h-20 mx-auto rounded-full bg-accent flex items-center justify-center mb-4 text-3xl">
              📄
            </div>
            <h2 className="text-2xl font-bold">Upload Prescription</h2>
            <p className="text-muted-foreground mt-2">
              Our pharmacist will verify and prepare your order.
            </p>
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 animate-fade-in">
        <AnimatePresence mode="wait">
          <motion.div
            key={view + (profileSubRoute || "") + (selectedMedicine || "")}
